...
circle_loop:
    ...
    mov A, R3           
    addc A, #04h        ; Add integer step (1) + carry from fraction
    mov R3, A           ; R3 now holds the exact table offset

    ; output for x-axis
    mov DPTR, #sine_table
    movc A, @A+DPTR     ; Fetch raw sine value using R3
    mov DPTR, #x_dac
    movx @DPTR, A       ; Write to X-DAC
    ...
    mov A, R5           
    addc A, #04h        ; Add integer step (1) + carry from fraction
    mov R5, A           ; R5 now holds the exact table offset

    ; y-axis output
    mov DPTR, #sine_table
    movc A, @A+DPTR     ; Fetch raw cosine value using R5
    mov DPTR, #y_dac 
    movx @DPTR, A       ; Write to Y-DAC
