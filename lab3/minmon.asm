...
jumtab:
   ...
   .dw readbt             ; command 'r' 12
   .dw badcmd             ; command 's' 13
   .dw badcmd             ; command 't' 14
   .dw badcmd             ; command 'u' 15
   .dw vector             ; command 'v' 16
   .dw writeb             ; command 'w' 17
...
;*****************************************************************
; command readbt
; this command reads a byte in external memory and prints on PC
;*****************************************************************

readbt:
    lcall getbyt            ; get upper byte
    mov R7, A               ; move from acc to R7
    lcall prthex            ; echo back

    lcall getbyt            ; get lower byte
    mov R6, A               ; move from acc to R6
    lcall prthex            ; echo back
    
    mov dph, R7             ; move low/high byte into DPTR
    mov dpl, R6             ;  

    lcall crlf              ; new line

    movx A, @DPTR           ; get value at DPTR
    lcall prthex            ; print val on screen

    ljmp endloop

;*****************************************************************
; command writeb
; this command writes a byte into external memory
;*****************************************************************
writeb: 
    lcall getbyt            ; get upper byte
    mov R7, A               ; move from acc to R7
    ...
    lcall getbyt            ; get new value
    mov R5, A               ; move from acc to R5
    lcall prthex

    mov dph, R7             ; move address to DPTR
    mov dpl, R6
...
;*****************************************************************
; command vector
; this command writes a vector
;*****************************************************************

vector:
   lcall getbyt            ; get MSB of the address
   mov R7, A               ; store MSB in R7 (Safe Register)
   lcall prthex            ; echo out MSB of address
   mov R6, #00h            ; start LSB in R6 at 00h (Safe Register)
vector_loop:
   lcall crlf              ; new line for the next address
   mov A, R7               ; print MSB
   lcall prthex
   mov A, R6               ; print LSB
   lcall prthex
   mov A, #' '             ; print space delimiter
   lcall sndchr

   lcall getbyt            ; get data byte. Enter aborts loop automatically!
   
   mov dph, R7             ; set DPTR pointer MSB
   mov dpl, R6             ; set DPTR pointer LSB
   movx @DPTR, A           ; write data to external memory

   lcall prthex            ; print the data byte

   cjne R6, #0FFh, increment ; check if we hit the 256th byte (FFh)
   ljmp endloop              ; exit if we did

increment:
   inc R6
   sjmp vector_loop          ; loop for next byte
